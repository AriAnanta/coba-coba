  ...
  try {
    ...
    if (!feedback) {
      ...
      feedback = await ProductionFeedback.create({
        ...
        productionId: queueData.productionId || generateUniqueId("PROD"),
        batchId: queueData.batchId,
        productId: queueData.productId || generateUniqueId("ITEM"),
        productName: queueData.productName || "Produk Tidak Diketahui",
        status: "pending",
        quantityOrdered: queueData.plannedQuantity || 0,
    ...
    switch (queueData.status) {
      ...
      case "completed":
        ...
        notificationMessage = `Produksi untuk ${queueData.productName} (Batch: ${queueData.batchId}) telah selesai dengan jumlah ${queueData.completedQuantity || 0} unit.`;
        break;
      case "paused":
      ...
      case "failed":
        ...
        notificationTitle = "Produksi Gagal";
        notificationMessage = `Produksi untuk ${queueData.productName} (Batch: ${queueData.batchId}) telah gagal.`;
        break;
      default:
        // Tidak perlu update status jika tidak ada perubahan yang relevan
        console.log(`Status tidak dikenali: ${queueData.status}, tidak ada perubahan yang dilakukan`);
        return;
    }

    // Hitung completion percentage berdasarkan jumlah yang diproduksi
    const completionPercentage = feedback.quantityOrdered > 0 
      ? ((queueData.completedQuantity || 0) / feedback.quantityOrdered) * 100 
      : 0;
    ...
    await feedback.update({
      ...
      endDate: queueData.status === "completed" ? (queueData.actualEndTime || new Date()) : feedback.endDate,
      quantityProduced: (feedback.quantityProduced || 0) + (queueData.completedQuantity || 0),
      quantityRejected: (feedback.quantityRejected || 0) + (queueData.defectQuantity || 0),
      completionPercentage: Math.min(completionPercentage, 100), // Pastikan tidak lebih dari 100%
    });

    // Buat notifikasi untuk perubahan status
    await createNotification(
      feedback.id, // Menggunakan ID feedback sebagai referensi
      notificationType,
      notificationTitle,
      notificationMessage,
      "all", // Kirim ke semua pengguna yang terkait
      "system", // Pengirim adalah sistem
      "medium"
    );

    console.log(
      `Berhasil memproses pesan dan membuat notifikasi untuk ${queueData.batchId}. Status: ${feedbackStatus}`
    );
  } catch (error) {
